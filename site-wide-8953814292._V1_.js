...
}function matchDepartments(searchTerm){if(!opt.isDigitalFeaturesEnabled&&!overrideDepartmentWeblab){return;
...
}if(isXOSuggestion(sg)){issMode=issModePrefix+"xo";
}else{if(isFallbackSuggestion(sg)){issMode=issModePrefix+"fb";
...
}function isXOSuggestion(sg){return(sg&&sg.source&&sg.source[0]=="xo");
...
si<s.length;
si++){if(s[si]==="fb"||s[si]==="xo"){if(size==1&&enableCatSuggestionImprovementWithoutKeyword()){redirectFirstSuggestion=true;
...
}}a(opt.protocol,"//",opt.src,"?","method=completion","&q=",encodeURIComponent(kw),"&search-alias=",alias,"&client=",opt.cid,"&mkt=",opt.mkt,"&fb=",opt.fb,"&xcat=",opt.xcat,"&cf=",opt.cf,"&x=updateISSCompletion");
...
suggestRequest=null;
if(!searchBox.hasFocus()){return;
}var cf=completion[3],cfi;
if(cf&&cf.length){for(cfi=0;
cfi<cf.length;
cfi++){if(cf[cfi].cf){cfFlag=cf[cfi].cf;
break;
}}}else{cfFlag=0;
...
}function isQIAFilterApplied(){return cfFlag==1;
}function haveCategorySuggestions(){return lastXCatSuggestionPosition>0||hasFallbackSuggestion;
}return{suggest:bindSuggest,keypress:bindKeypress,submit:bindSubmit,blur:blur,keyword:keyword,merchant:merchant,searchAlias:searchAlias,searchNode:searchNode,stop:stop,start:start,encoding:encoding,focus:focus,offset:offset,keydown:keydown,isImeEnhUsed:isImeEnhUsed,triggerImeEnh:triggerImeEnh,isQIAFilterApplied:isQIAFilterApplied,haveCategorySuggestions:haveCategorySuggestions,isDigitalFeaturesEnabled:function(){return opt.isDigitalFeaturesEnabled===1;
